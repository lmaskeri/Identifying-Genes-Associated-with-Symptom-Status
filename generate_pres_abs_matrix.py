...
strain_numbers = open("./strainNumbers.txt").read().split("\n") #pulling out the strain numbers 
...
#Making the csv file in order to write the presence absence matrix out to
with open("presence_absence.csv", "w", newline = "") as out_file:
    writer = csv.writer(out_file)
    #making the header with all of the homologous gene names
    header = ["Strain Number"]
    for gene in homologous_genes:
        header.append(gene)
    writer.writerow(header) #writing the header row to the csv file
    for i in range(len(strain_numbers)): 
        strain_row = [strain_numbers[i]] #placing the strain number as the first item in the row
        ...
        writer.writerow(strain_row) #writing the presence absence information from this strain to the csv file as a row
