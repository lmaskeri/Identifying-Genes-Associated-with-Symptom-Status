...
def findGeneName(name):
    ...
    with open("./cluster_dir/"+name) as cluster: #open centroid file
        ...
        genes.append(gene_name) #appending the final homologous gene name to a list
    counter = 0
    num = genes[0]
    ...
    for i in genes:
        ...
        if(curr_freq>counter):
            ...
            num = i
    return num
def makeResultsList(code,pos_neg,pos_gene_name,neg_gene_name,count):
    pos_name = pos_neg[0]
    pos_corr = pos_neg[1]
    ...
    results_list = [code, pos_name, pos_corr,pos_gene_name, neg_name, neg_corr,neg_gene_name,count]
    return results_list
